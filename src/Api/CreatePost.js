...
app.post('/api/add_activity', function(req, res){
    ...
    if (!start_time) {
      res.status(result.requestbad).send('invalid parameters: no start time');
      return;
    }

    if (!end_time) {
      res.status(result.requestbad).send('invalid parameters: no end time');
      return;
    }

    if (!location_name) {
      res.status(result.requestbad).send('invalid parameters: no location name');
      return;
    }
    ...
    if (!host) {
      res.status(result.requestbad).send('invalid parameters: no host');
      return;
    }
    ...
    if (title.length > 500) {
      res.status(result.requestbad).send('invalid parameters: title too long');
      return;
    }

    if (details.length > 1000) {
      res.status(result.requestbad).send('invalid parameters: details too long');
      return;
    }
    ...
    */
    ...
    // fake data. uncomment to have fake data on new posts
