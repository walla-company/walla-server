...
app.post('/api/update_user_goal2', function(req, res){
  ...
  if(!goal2){
      res.status(result.requestbad).send('invalid parameters: no goal2');
      return;
  }

  databaseref.child('schools/' + school_identifier + '/users/' + uid + '/goal2').set(goal2);

  pointsManager.addProfileCompletionPointsToUser(school_identifier, uid);

  res.status(result.requestsuccessful).send('goal2 updated');
